...
A key result of this work is the numerical demonstration in Appendix~\ref{app:simulation} that Lévy-tailed redshift statistics emerge from the RSVP field equations without being imposed. A near-critical scalar field with physically motivated parameters generates domain walls that act as topological hotspots for the weighted relaxation functional~\eqref{eq:R_weighted}. Photon trajectories accumulating $\kappa\!\int\!\mathcal{R}_w\,d\lambda$ across these hotspots produce a redshift distribution with Hill tail index $\hat{\mu} = 1.469 < 2$ (Lévy regime), kurtosis of 2471, and a causal correlation between topological encounter count and redshift scatter (Spearman $r = 0.365$, $p = 2.2\times10^{-157}$). This is the first computational demonstration that the core claims of Sections~\ref{sec:singularity} and~\ref{sec:spherepop}---that redshift is driven by irreversible topological events rather than metric expansion---are mechanistically realised within the field dynamics.

Several important directions remain open. First, the symmetry reduction of Section~\ref{sec:symmetry} reduces the $\alpha_i$ to a single ratio $\xi_{\mathrm{RSVP}}$; fixing this parameter against observational data, particularly the Hubble tension split between CMB and late-universe probes, is the most urgent empirical task. Second, the calibration of $\kappa$ against known cosmological data would constrain the energetic cost per irreversible event and potentially link the RSVP framework to quantum information-theoretic bounds. Third, an explicit fit of the phenomenological model (Appendix~\ref{app:pheno}) against DESI DR2 BAO, Pantheon+, and Planck lensing data would test whether RSVP can reproduce current constraints while predicting distinguishable residuals in higher-order statistics. Fourth, the KES framework of Section~\ref{sec:kes} connects cosmological redshift to possibility contraction $\dot{\mathcal{V}}_\Omega$; the phantom-to-quintessence transition observed by Zhang et al.\ corresponds in this language to accelerating followed by decelerating contraction, a prediction that could be tested through epoch-resolved analyses of BAO and supernova residuals.
...
\begin{itemize}[leftmargin=2em]
\item \textbf{Hill tail-index estimate}: $\hat{\mu} = 1.469 < 2$, placing the
  distribution in the Lévy regime (infinite theoretical variance). This is
  consistent with Theorem~\ref{thm:levy}.
\item \textbf{Kurtosis}: $2471$ (full distribution),
  far exceeding the Gaussian value of $3.0$, driven by rare burst events.
\item \textbf{Normality rejected}: D'Agostino--Pearson test $p \ll 10^{-300}$ (numerical underflow).
\item \textbf{Hotspot correlation}: Spearman $r = 0.365$ between hotspot
  encounter count and $\log(1+z)$, $p = 2.2 \times 10^{-157}$, confirming
...
\begin{figure}[h!]
\centering
\includegraphics[width=\textwidth]{rsvp_simulation.png}
...
with Hill tail index $\hat{\mu} = 1.469 < 2$ (Lévy regime), kurtosis $= 2471$,
and normality rejected at $p \ll 10^{-300}$.
Panel (E) confirms that topological hotspot encounters are the causal driver
of redshift scatter (Spearman $r = 0.365$, $p = 2.2\times10^{-157}$).}
...
The parameter $\lambda_\Phi = 0.03$ places the system near but not at a
